...
// ── Font toggle ───────────────────────────────────────────────
const APP_VERSION = 'v4.98';   // MUST match service-worker CACHE_NAME (self-heal compares them). Bump as v1.13, v1.14…
...
// 畫作一律有分級：沒有「不分級」桶，未設定（舊資料/留白）一律落在吐真劑。
function renderImageCatOptions(sel, current, keepMqj) {
  ...
  sel.value = cats.includes(current) ? current : '吐真劑';
}
let shelfCat = '';        // '' = 全部
...
async function saveEditWork() {
  ...
  if (editWork.kind === 'image') {
    ...
    body.category = (document.getElementById('editwork-cat') || {}).value || '吐真劑';
    try {
    ...
    box.innerHTML = `
      ...
      <div style="font-size:12.5px;color:var(--ink);background:var(--parchment);border:1px solid var(--gold-lt);border-radius:6px;padding:8px 10px;line-height:1.7">${
        res.opens_at
          ...
          : `${ic('ic-check',12)} 閱報須知頁已自動亮起領取入口——先前貼出去的閱報須知連結<b>即刻生效，不需再發新連結</b>。`
      }</div>
